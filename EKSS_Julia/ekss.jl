...
"""
EKSS Module. Clusters `N` data points via Ensemble K-Subspaces Clustering (EKSS) Algorithm.
"""
module EKSS
...
export EKSSResult, fit, fit_predict
@compat public ekss_affinity, embedding
## Algorithm: EKSS
...
function ekss(
    ...
    N = size(X, 2)
    q = isnothing(q) ? max(300, round(Int, 0.01 * N)) : q
    ...
    # Run KSS Ensemble
    runs = Vector{KSS.KSSResult}(undef, nruns)
...
function ekss_base_run(
    ...
    Uinit_r = [KSS.randsubspace(rng_r, size(X, 1), d) for _ in 1:Kbar]
    return KSS.kss(X, dvec; maxiters = maxiters, rng = rng_r, Uinit = Uinit_r)
end
...
"""
    fit(X, d, K; kwargs...) -> EKSSResult

Run **Ensemble K-Subspaces Clustering (EKSS)** on the data matrix `X` and return a
[`EKSSResult`](@ref) containing the co-association matrix, spectral embedding, and labels.

# Arguments
- `X::AbstractMatrix{<:Real}`: Data matrix of size `D × N` (columns are points).
- `d::Integer`: Subspace dimensions for each cluster. 
...
# Returns
- `res::EKSSResult` with fields:
  - `res.coassoc`, `res.embedding`, `res.assignments`, `res.kmeans_runs`
"""
function fit(X::AbstractMatrix{<:Real}, d::Integer, K::Integer; kwargs...)
    return ekss(X, d, K; kwargs...)
end
...
"""
function fit_predict(X::AbstractMatrix{<:Real}, d::Integer, K::Integer; kwargs...)
    res = ekss(X, d, K; kwargs...)
