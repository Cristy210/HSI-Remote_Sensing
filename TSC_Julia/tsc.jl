"""
TSC Module. Clusters `N` data points via Thresholded Subspace Clustering (TSC) Algorithm.
"""
module TSC
...
export TSCResult, tsc, fit, fit_predict
@compat public tsc_affinity, tsc_embedding

mutable struct TSCResult{
...
"""
    fit(X, K; kwargs...) -> TSCResult

Run **Thresholded Subspace Clustering (TSC)** on the data matrix `X` and return a
[`TSCResult`](@ref) containing the affinity matrix, spectral embedding, and labels.

# Arguments
- `X::AbstractMatrix{<:Real}`: Data matrix of size `D × N` (columns are points).
- `K::Integer`: number of clusters.

# Keyword Arguments
All keyword arguments are forwarded to [`tsc`](@ref), including:
- `max_nz`: maximum number of neighbors kept per point in the thresholded affinity
- `max_chunksize`: chunk size for affinity computation
- `rng`: RNG passed to K-means
- `kmeans_nruns`: number of K-means restarts
- `kmeans_opts`: additional keyword options forwarded to `kmeans`

# Returns
- `res::TSCResult` with fields:
  - `res.affinity`, `res.embedding`, `res.assignments`, `res.kmeans_runs`
"""
function fit(X::AbstractMatrix{<:Real}, K::Integer; kwargs...)
    return tsc(X, K; kwargs...)
end

"""
    fit_predict(X, K; kwargs...) -> assignments

Convenience wrapper: runs `fit(X, K; kwargs...)` and returns only the cluster labels.
"""
function fit_predict(X::AbstractMatrix{<:Real}, K::Integer; kwargs...)
    res = tsc(X, K; kwargs...)
