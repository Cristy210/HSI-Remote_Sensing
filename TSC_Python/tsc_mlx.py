...
class TSC_MLX:
    ...
    use_gpu : bool, default=True
        If True, use MLX GPU stream for similarity computation.
    ...
    def _to_mlx_column_major(self, X) -> Tuple[Array, int, int]:
        """
        Convert input X to MLX array of shape (D, N).
        ...
        """
        dtype = mx.float32 if self.use_gpu else mx.float64
        X_np = np.asarray(X, dtype=np.float32 if self.use_gpu else np.float64)
        if X_np.ndim != 2:
            raise ValueError("Input data X must be 2D with shape (n_features, n_samples).")

        if self.verbose >= 2:
            backend = "gpu" if self.use_gpu else "cpu"
            print(f"[TSC] Converting input to MLX array on {backend} with dtype={dtype}, shape={X_np.shape}")
        X_mx = mx.array(X_np, dtype=dtype)
        D, N = X_mx.shape
        return X_mx, D, N
    ...
    def _build_affinity(self, X: Array):
        """
        ...
        try:
            ...
            raise ImportError(
                "SciPy is required for sparse affinity construction. "
        ...
        data = np.empty(nnz_est, dtype=np.float32)
        ...
        if self.verbose >= 1:
            backend = "GPU" if self.use_gpu else "CPU"
            print(
                f"[TSC] Building affinity matrix | N = {N}, q={q}, chunksize={chunksize}"
                f"n_chunks={n_chunks}, backend={backend}")
        if self.verbose >= 2:
    ...
    def _spectral_embedding(self, A):
        """
        ...
        try:
            ...
            raise ImportError(
                "SciPy is required for spectral embedding. "
                "Install it with: pip install scipy"
            ) from e
        ...
        if N == 1:
            ...
            vecs = np.ones((1, 1), dtype=np.float32)
        elif nev >= N - 1:
            dense = L.toarray()
        ...
        if self.n_clusters is None and self.verbose >= 1:
        ...
        embedding = np.asarray(vecs[:, :K], dtype=np.float32)
